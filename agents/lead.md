...
- **Monitoring backoff:** while waiting on workers, use explicit waits with checks: `sleep 30 && ov mail check --agent $OVERSTORY_AGENT_NAME`, then 60s, 120s, and 300s cap; reset to 30s when new activity appears.
